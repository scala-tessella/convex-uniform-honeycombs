...
sbt -Dcerts test            # + regenerate the eighteen cited certificates into certs/ (includes the above)
```
...
| **The exact certificates** | `ExactCertificatesSpec` | All 34 star models and all 28 periodization certificates re-verified in **exact arithmetic over $\mathbb{Q}(\sqrt2,\sqrt3)$** via the internal basis (nothing is ever normalized), every gluing's ring agreement checked cell by cell with the face **roles** (equality of placed cells, by the cell rigidity lemma), with recognized Gram entries confined inside interval enclosures of width $<6\cdot10^{-9}$, collision-free periodic balls of **515 to 4521** vertices that are box-periodic and closed, on which every generator is checked to act as an exact symmetry, and the two doubled species separated by **exact** canonical fingerprints; germ forcing replayed exactly on every skeleton with a third verdict, **excluded** — the coset placement fails the exact ring agreement with roles, so the skeleton holds no gluing (10 of 41; 27 forced, 4 open) — and the numeric atlas read exactly, genuine gluings against atlas size at every tiling-vertex; and, opt-in (`-DexactCoherence`, always under `-Dcerts`), the **coherence of every accepted pattern** with its class — own exact certificate, exact alignment by a stabilizer element, the representative's lattice transported and checked — within the caps and, on the ten skeletons germ forcing leaves open, beyond them: every exhaustion re-enumerated uncapped and every accepted pattern cohered exactly. Every positive certificate on the critical path is exact; what stays numeric is the enumerations' negative decisions. Certificate: `exact-certificates.txt`. |
...
| **Rigidity of the core cells** — Lemma A.2 and the cell rigidity lemma | `CoreCellsSpec` | For each of the 13 core cells, the combinatorial automorphism group of the face lattice and the geometric symmetry group have the **same order** (24, 48, 48, 24, 48, 48, 48, 48, 48, 12, 24, 32, 48), so every combinatorial automorphism is an isometry; the 13 face-size multisets are pairwise distinct. The three finite inputs of the cell rigidity lemma — face roles by size except the rco's six axial and twelve belt squares, the bisector reflection of every edge a symmetry, transitivity on the edge triples of each ordered role pair — hold on all 13, and the rco counterexample to the role-free statement is asserted. Certificate: `core-cells.txt`. |
| **The developability gap, witnessed** — Example 6.4 | `SnubCollisionSpec` | On the snub-trihexagonal lift an (R1)+(R2)-consistent pattern whose two-step words **4·1 and 6·0** reach the same position with stabilizer-inequivalent point parts. Certificate: `snub-collision.txt`. |
| The certificates | `CertificatesSpec` | Not a claim: regenerates the eighteen cited certificate files into `certs/`. *(opt-in: `-Dcerts`)* |
...
Pinned to `research-core 0.12.0` (an immutable Central release), archived as
[10.5281/zenodo.22862797](https://doi.org/10.5281/zenodo.22862797). That archived snapshot — not the
