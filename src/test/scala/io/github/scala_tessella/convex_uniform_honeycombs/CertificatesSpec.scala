...
/** Regenerates the sixteen plain-text certificates the paper cites, into `certs/`.
  *
  * Opt-in (`sbt -Dcerts test`) for one reason only: it writes files. Every claim these certificates record is
  * asserted by the specs of the fast tier, which run on every build without touching the filesystem — the
  * certificates are the human-readable rendering of the same computation, not a second source of truth.
  *
